...
$user_id =$_SESSION['user_id'] ;
 $sql = "SELECT * FROM movies  LEFT JOIN movie_ratings ON movies.id = movie_ratings.movie_id WHERE movie_ratings.user_id = $user_id GROUP BY movie_ratings.movie_id";
...
if(mysqli_num_rows($query)>0){
    while($movies = mysqli_fetch_assoc($query)){
        $sql = "SELECT *,SUM(rating) AS total_rating,COUNT(*) as total_records FROM movie_ratings WHERE movie_id = $movies[movie_id]";
